...
const Reviews = ({ roundedRating, reviewCount }: { roundedRating: number; reviewCount: any }) => (
  <ReviewText data-testid="rating-reviews">
    &nbsp; &nbsp; {roundedRating} ({reviewCount} reviews)
  </ReviewText>
);
...
const Rating = ({ roundedRating, reviewCount }: { roundedRating: number; reviewCount: any }) => (
  <RatingContainer data-testid="general-rating">
    <StarImage
      starNumber={1}
      starType={roundedRating <= 0.4 ? starEmpty : roundedRating <= 0.9 ? starHalf : starFull}
    />
    <StarImage
      starNumber={2}
      starType={roundedRating <= 1.4 ? starEmpty : roundedRating <= 1.9 ? starHalf : starFull}
    />
    <StarImage
      starNumber={3}
      starType={roundedRating <= 2.4 ? starEmpty : roundedRating <= 2.9 ? starHalf : starFull}
    />
    <StarImage
      starNumber={4}
      starType={roundedRating <= 3.4 ? starEmpty : roundedRating <= 3.9 ? starHalf : starFull}
    />
    <StarImage
      starNumber={5}
      starType={roundedRating <= 4.4 ? starEmpty : roundedRating <= 4.9 ? starHalf : starFull}
    />
    <Reviews roundedRating={roundedRating} reviewCount={reviewCount} />
  </RatingContainer>
);

function RenderRating({ productRating }: { productRating: RatingType }) {
  const { count, rate } = productRating;
  const roundedRating = Math.round(rate * 10) / 10;

  return <Rating roundedRating={roundedRating} reviewCount={count} />;
}
